...
`set_state` and `delete_state` accept `expected_version`. Tombstones preserve revision history.
...
Back up real databases before upgrading; the additive runtime schema currently has version 1.
