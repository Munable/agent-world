...
class SetRoleStatusRequest(StrictModel):
    status: str


class IssueJoinTicketRequest(StrictModel):
...
def create_onboarding_app(
    ...
    *,
    ...
    def require_operator(value: str | None) -> None:
        ...
        if not hmac.compare_digest(value.encode(), operator_key.encode()):
    ...
    def revoke_identity_token(
        ...
        require_operator(x_operator_key)
        changed = runtime.revoke_identity_token(token_id, expected_universe=universe)
        if not changed:
...
def _from_environment():
    ...
    ttl = os.getenv("WORLD_IDENTITY_TTL_SECONDS")
    return create_onboarding_app(
        db,
        universe,
        ...
        identity_ttl_seconds=float(ttl) if ttl else None,
    )
